...
<table>
    ...
    <tr>
        <td>9.10 设置传感器类型 0x39</td>
        <td>0x39</td>
        <td>settingFlag:设置标志（0设置，1查询） sensorType:传感器类型（0：绝压传感器，0xaa:表压传感器）decimalPlace:小数位：（0:1位小数，0xaa:两位小数,0XBB:三位小数）port:设置进口或出口 (0进口，1出口)</td>
        <td>同上,不需要显示设置标志</td>
    </tr>
</table>