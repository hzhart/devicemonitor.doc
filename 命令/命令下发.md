...
<table>
    ...
    <tr>
        ...
        <td>-</td>
        <td>-</td>
        <td>参数：{"enable":是否启用报警采集 1：启用 0：不启用, "period":采集时长周期数N(1~50),"interval":采集间隔(秒), "duration": 采集时长(分)} 返回：result: 0x00(成功)</td>
    </tr>
    <tr>
    ...
    <tr>
        ...
        <td>-</td>
        <td>-</td>
        <td>参数：{"enable":是否启用连续工作模式 1：启用 0：不启用, "dataGatherInterval":采集间隔(秒),"workTime":工作时长(分), "reportInterval": 上报间隔(分)} 返回：result: 0x00(成功)</td>
    </tr>
</table>
