...
<table>
    ...
    <tr>
        ...
        <td>0x6F</td>
        <td>liquidDistanceCheck: 液距检测启用状态(0：不启用 1：启用)，liquidDistance: 液位距离(米)，
angleCheck：倾斜角检测启用状态 angle：倾斜角度 
vibrationAlarmEnable：震动报警启用
displacementAlarmEnable：位移报警启用
displacementAlarmDistance：位移报警距离(米)
photoEnable：拍照启用(1:启用 0:不启用)
</td>
        <td>-</td>
        <td>成功：0xFF或0x00</td>
    </tr>
